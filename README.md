...
2. 常用命令类提示词
   ...
   - 检查：自动检查代码中存在的问题，整理问题并提交给用户进行确认，根据反馈执行修复：
     * 开始检查前，先生成检查记录文件，文件名：[./qreports/check-<日期时间>.md]
     * 检查记录文件中，需要记录检查的目录、文件、问题等
     * 检查过程中，每次整理超过5个问题，马上停止检查，等待用户确认
     * 用户确认并修改后，提交GIT，然后继续检查
     * 待检查问题结束后，自动生成问题报告文件，文件名：[./qreports/report-<日期时间>.md]（日期时间与检查记录文件一致）,报告内容尽量简明，汇总本次检查问题分类，问题简明清单及要点描述，以及对应概要说明
     ...
     * 自动更新文件，不需要用户确认，并同步提交到GIT
   - 日志搜索：
     * 根据用户输入，搜索关联度最高的前10条记录，分别显示：序号、日志文件名、日志时间、日志摘要
     * 并提示用户选择要查看的日志，用户选择后，自动显示日志文件内容并定位且高亮显示对应行
   - TODO搜索：
...
7. 履历管理
   ...
   - 日志按照天来记录，文件名：[./history/history-<日期时间>.md]
   - 记录包含： 请求时间、请求内容、请求结果、请求耗时
   - 自动更新文件，不需要用户确认，并同步提交到GIT

8. TODO管理
   - 在根目录下记录TODO清单，文件名：[./TODO.md]
   - 每次同用户交互过程中，用户对暂不处理的问题，请自动记录到TODO清单中
   - TODO记录内容：
     * 序号（从1开始，自动增加）
     * 标题
     * 记录时间
