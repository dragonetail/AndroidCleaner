...
### 交互原则（重要，请一定要遵守）
- 请不要直接追加任何新的功能，仅仅对现有的功能进行修复、健壮性强化、日志补充、注释完善等。
- 如果要追加和新增新的功能，一定要由我来发起，或者询问我等我确认后再追加，特别是已经完成的功能不要追加新的功能改变和设计初衷，后面又要重新Rework或者删除。
- 
...
6. 录音包含：
  ...
  - 正常状态下，录音记录列表显示的录音项包括：
    ...
    - 选择状态下：
      - 录音项后面的播放按钮变化显示为选择框
      - 在右上角应该有【关闭×】（系统回退）来退出选择状态
      - 选择状态下菜单应该浮现在下方，菜单具有功能【分享、转文字、删除、全选、重命名】，如果菜单位置有限，使用四个点的更多按钮展示更多菜单内容
      - 在上方应该有整体表示选择了多少个文件（可以直接更改标题行的内容，也可以另外一行
  - 在标题栏右边有一个四个点的图标，表示更多，中间追加三个常用过滤的二级菜单：
    - 时间： 全部、今天、最近7天、最近30天、最近90天
    - 时长： 全部、1分钟以内、5分钟以内、30分钟以内、2个小时以内、更长
    - 排序： 时间降序、时间升序、大小降序、大小升序
7. 通话包含：
  - 通话记录列表
  - 通话播放
8. 联系人包含：
